...
ha-foodinator pushes events into an existing Home Assistant "Local Calendar"
...
ha-foodinator container runs (the same LAN, or through whatever networking you use
...
meal-plan entry changes or is cancelled *after* ha-foodinator has already pushed it
to HA, the stale event will **not** be automatically corrected or removed - it needs
to be edited or deleted manually in the HA calendar UI. ha-foodinator's sync horizon
